...
class DataGenerator:
    ...
    def generate_states(self, p0, q0):
        ...
        return vel_w, acc_w, goal_W, vel_b, acc_b, goal_b
    ...
    def generate_and_save(self, npz_path):
        ...
        for i in tqdm(range(self.dataset_size)):
            # 初始化优化器
            vel_w0, acc_w0, goal_w, vel_b0, acc_b0, goal_b = self.generate_states(self.p0[i], self.q0[i])
            ...
            v0_all[i] = vel_b0
            a0_all[i] = acc_b0
            p1_all[i] = np.array(p1).flatten()
            v1_all[i] = np.array(v1).flatten()
            a1_all[i] = np.array(a1).flatten()
            goal_all[i] = goal_b
